...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchSuggestedRepositories = async () => {
      try {
        const response = await fetch(`https://dev-sync-backend.vercel.app/repo/all`)
        const data = await response.json()
  ...
  const handleCreateRepository = async () => {
    ...
    try {
      const userId = localStorage.getItem("userId")
      const response = await fetch("https://dev-sync-backend.vercel.app/repo/create", {
        method: "POST",
        headers: {
      ...
      const updatedRepos = await fetch(`https://dev-sync-backend.vercel.app/repo/user/${userId}`).then(res => res.json())
