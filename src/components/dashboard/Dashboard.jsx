...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchSuggestedRepositories = async () => {
      try {
        const response = await fetch(`http://34.227.150.165:3000/repo/all`)
        const data = await response.json()
  ...
  const handleCreateRepository = async () => {
    ...
    try {
      const userId = localStorage.getItem("userId")
      const response = await fetch("http://34.227.150.165:3000/repo/create", {
        method: "POST",
        headers: {
      ...
      const updatedRepos = await fetch(`http://34.227.150.165:3000/repo/user/${userId}`).then(res => res.json())
